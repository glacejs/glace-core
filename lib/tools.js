...
const printTests = filter => {
    ...
    if (!CONF.test.cases.length) {
        console.log("No tests are found".yellow);
        return;
    }

    CONF.test.cases.forEach((testCase, i) => {
        if (filter && !U.textContains(testCase.name, filter)) return;
        console.log(d(`${i+1}. ${testCase.name}`));
    });
};
...
const printPlugins = () => {
    ...
    pluginsList.forEach((plugin, i) => {
        console.log(`${i+1}. ${plugin.name}`.yellow, plugin.path);
    });
};
