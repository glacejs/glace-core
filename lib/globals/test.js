...
const _test = (names => {
    return (name, fixtures, opts, func) => {
        ...
        const testOpts = {};
        testOpts.chunkRetry = U.defVal(opts.chunkRetry, CONF.chunk.retries, 0);
        testOpts.chunkTimeout = U.defVal(opts.chunkTimeout);
        ...
        testFunc({ testCase, fixtures, testOpts, func });
    };
})([]);

const testFunc = ({ testCase, fixtures, testOpts, func }) => {
    fixtures = [initTestFixture(testCase)].concat(fixtures);
    const scopeType = new ScopeType(testCase.name).setType("test");

    scope(scopeType, fixtures, testOpts, () => {
        func();
    });
};
