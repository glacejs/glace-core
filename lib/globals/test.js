 ...
 */
var setLog = () => {
    var testName = CONF.test.curCase ? _.kebabCase(CONF.test.curCase.name) : "";
    var logFile = path.resolve(CONF.report.logsDir, testName, "logs", "test.log");
    LOG.setFile(logFile);
};
...
var baseTest = (names => {
    return (name, opts, fixtures, func) => {
        ...
        opts.retry = U.defVal(opts.retry, CONF.test.retries, 0);
        opts.chunkRetry = U.defVal(opts.chunkRetry, CONF.test.chunkRetries, 0);
