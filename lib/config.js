 ...
 * @prop {string} [reportsDir=cwd/report] - Path to report folder.
...
if (U.config.__selftest) config = {}; // not affect global config in self-tests
config = _.merge(config, {
    ...
    reportsDir: path.resolve(U.cwd, U.defVal(args.report, "report")),
    isRunPassed: false,
    languages: []
});
...
config.checkTestNames = !args.dontCheckNames;
...
config.failedTestsPath = path.resolve(config.reportsDir, U.defVal(args.failedTestsPath, "failed-tests.json"));
...
config.xunit.path = path.resolve(config.reportsDir, U.defVal(args.xunitPath, "xunit.xml"));
...
config.allure.dir = path.resolve(config.reportsDir, U.defVal(args.allureDir, "allure"));
