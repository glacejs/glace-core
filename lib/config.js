 ...
 * @prop {boolean} [session.noErrors=false] - Flag to define that session
 * errors happened
...
const get_targets = () => {
    ...
    return _.flatten(tt.map(t => glob.sync(t))).map(t => path.resolve(U.cwd, t));
};
