...
var GlaceReporter = function (runner) {
    ...
    runner.on("end", () => {
        ...
        if (!CONF.testCases.filter(t => t.status === TestCase.FAILED).length && !sessErrsNum) {
    ...
    runner.on("suite", mochaSuite => {
        for (var reporter of reporters) {
            if (CONF.testSuites.includes(mochaSuite.title)) {
    ...
    runner.on("pass", mochaTest => {
        if (CONF.curTestCase && CONF.curTestCase.skipChunk === mochaTest.title) {
            mochaTest.state = "skipped";
            CONF.curTestCase.skipChunk = null;
        }
    ...
    runner.on("fail", (mochaTest, err) => {
        if (CONF.curTestCase) {
            ...
            if (!_.isEmpty(CONF.curTestCase.testParams)) {
                errMsg += "\n" + util.format(CONF.curTestCase.testParams);
            }
            if (err.message) {
                errMsg += "\nmessage: " + err.message;
            }
