...
fse.mkdirs(CONF.logsDir);
var report = fs.createWriteStream(
    path.resolve(CONF.logsDir, "stdout.log"), { flags : "w" });
...
var epilogue = () => {
    ...
    if (fs.existsSync(CONF.failedTestsPath)) {
        try {
            fs.unlinkSync(CONF.failedTestsPath);
        } catch (e) {
            LOG.error(`Can't remove file '${CONF.failedTestsPath}'`, e);
        }
    };
    ...
    if (failedTests.length) {
        ...
        try {
            fs.writeFileSync(
                CONF.failedTestsPath,
                JSON.stringify(failedTestsJson, null, "  "));
        } catch (e) {
            LOG.error(`Can't write file '${CONF.failedTestsPath}'`, e);
        }
    }
...
module.exports = {
    ...
    fail: (chunk, err) => {
        ...
        if (!CONF.curTestCase) {
            ...
            errMsg = _.last(CONF.curTestCase.errors);
        };
