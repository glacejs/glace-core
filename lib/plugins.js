...
var getPlugins = module.exports.get = () => {
    if (!systemPlugins) {
        ...
        if (CONF.plugins.dir) {
            expect(fs.existsSync(CONF.plugins.dir),
                `Plugins folder '${CONF.plugins.dir}' doesn't exist`)
                .to.be.true;

            for (fileName of fs.readdirSync(CONF.plugins.dir).reverse()) {
        ...
        if (!CONF.plugins.disableDefault) {
